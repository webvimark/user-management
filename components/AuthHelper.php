...
class AuthHelper
{
	...
	/**
	 * Get cure small array like: ['role1', 'role2', etc]
	 *
	 * @param int  $userId
	 ...
	 * @return array
	 */
	public static function getRolesAsArray($userId, $withChildren = false)
	{
		$result = [];

		$items = Yii::$app->authManager->getRolesByUser($userId);
	...
	/**
	 * Get cure small array like: ['permission1', 'permission2', etc]
	 *
	 * @param int $userId
	 *
	 * @return array
	 */
	public static function getPermissionsAsArray($userId)
	{
		$result = [];

		$items = self::separateRoutesAndPermissions(Yii::$app->authManager->getPermissionsByUser($userId))->permissions;
	 ...
	 * @return array
	 */
	public static function getCalculatedRoutesAsArray($userId)
	{
		$result = [];

		$routes = self::separateRoutesAndPermissions(Yii::$app->authManager->getPermissionsByUser($userId))->routes;

		$allRoutes = self::separateRoutesAndPermissions(Yii::$app->authManager->getPermissions())->routes;

		foreach ($allRoutes as $name => $item)
		{
			foreach ($routes as $assignedName => $assignedItem)
			{
				if ( self::isChildRoute($name, $assignedName) )
				{
					$result[] = $name;
				}
			}
	 ...
	 * @return bool
	 */
	public static function isChildRoute($route, $assignedRoute)
	{
		...
		if ( substr($assignedRoute, -2) == '/*' )
		{
			$assignedRoute = rtrim($assignedRoute, '*');

			if ( strpos($route, $assignedRoute) === 0 )
			{
				return true;
			}
		}
